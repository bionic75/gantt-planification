...
`, (err) => {
    ...
    `, (err) => {
        if (err) {
            ...
            // Migration: ajouter la colonne user_id si elle n'existe pas (pour anciennes BDD)
            database.run(`ALTER TABLE deplacements ADD COLUMN user_id INTEGER`, (err) => {
                if (err && !err.message.includes('duplicate column')) {
                    // Ignorer l'erreur si la colonne existe déjà
                }
            });
            // Migration: ajouter la colonne commentaire si elle n'existe pas
...
// Route de diagnostic pour les déplacements
app.get('/api/deplacements/diagnostic', requireAdmin, (req, res) => {
    database.all(`SELECT * FROM deplacements`, [], (err, rows) => {
        if (err) {
            return res.status(500).json({ error: err.message });
        }
        ...
        const stats = {
            total: rows.length,
            avec_user_id: rows.filter(r => r.user_id != null).length,
            sans_user_id: rows.filter(r => r.user_id == null).length,
            avec_amoa_ced_id: rows.filter(r => r.amoa_ced_id != null).length,
            sans_amoa_ced_id: rows.filter(r => r.amoa_ced_id == null).length,
            user_ids_uniques: [...new Set(rows.map(r => r.user_id))],
            deplacements: rows
        };
